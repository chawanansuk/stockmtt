...
export async function getTransactions(opts = {}) {
  ...
  const vals = [];
  let i = 1;
  if (before) { where.push(`id < $${i++}`); vals.push(Number(before)); }
  if (type) { where.push(`type = $${i++}`); vals.push(type); }
  if (productId) { where.push(`items @> $${i++}::jsonb`); vals.push(JSON.stringify([{ productId: Number(productId) }])); }
  if (from) { where.push(`created_at >= $${i++}`); vals.push(new Date(from + 'T00:00:00')); }
  if (to) { const d = new Date(to + 'T00:00:00'); d.setDate(d.getDate() + 1); where.push(`created_at < $${i++}`); vals.push(d); }
...
export async function updateProduct(id, fields = {}, actor = '') {
  ...
  try {
    ...
    if (stockChanged) {
      ...
      const item = { productId: updated.id, name: updated.name, category: updated.category, quantity: delta, before: before.stock, after: updated.stock };
      await client.query(
  ...
  try {
    ...
    for (const it of items) {
      ...
      const r = await client.query(
        'UPDATE products SET stock = stock + $1 WHERE id=$2 AND deleted=false RETURNING id,name,category,stock',
...
// ส่งออกข้อมูลทุกตารางสำหรับสำรอง (รวมสินค้าที่ถูกลบไว้ด้วย เผื่อกู้คืน)
