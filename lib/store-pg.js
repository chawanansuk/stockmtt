...
export async function commit({ items, note = '', date = '', type = 'deduct', actor = '' }) {
  ...
  const r = await p.query(
    ...
    [id, type, date, note, actor, JSON.stringify(applied)]
  );
...
export async function voidTransaction(id, by = '') {
  ...
  const r = await p.query('UPDATE transactions SET voided=true, voided_at=now(), voided_by=$2 WHERE id=$1 RETURNING *', [Number(id), by]);
  return mapTx(r.rows[0]);
}
